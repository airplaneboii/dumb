...
class LittleGhostie(DumbAgent):
    def get_features(self, game_state, action):
        ...
        pacmans = [enemy for enemy in enemies if not enemy.is_pacman and enemy.get_position() is not None]
        ghosts = [enemy for enemy in enemies if enemy.is_pacman and enemy.get_position() is not None]
        #print(len(pacmans))
...
#                #if len(enemy_food) > 2:
            features['ispacman'] = 0 # delete this
            ...
            if len(pacmans) > 0:
                ...
                features['closest_food'] = food_path
